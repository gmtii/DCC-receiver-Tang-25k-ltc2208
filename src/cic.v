...
//------------------------------------------------------------------------------
//                            output rounding (NORMALIZED for /8)
//------------------------------------------------------------------------------
// STAGES=3, DECIMATION=8 => GROWTH = 3*log2(8) = 9
localparam integer GROWTH = 9;

wire signed [ACC_WIDTH-1:0] y      = comb_data[STAGES];
wire signed [ACC_WIDTH-1:0] y_norm = y >>> GROWTH;

// Recorte a OUT_WIDTH bits desde el valor ya normalizado
wire signed [OUT_WIDTH-1:0] y_slice = y_norm[OUT_WIDTH-1:0];

// bit de redondeo justo debajo
wire rb = y_norm[OUT_WIDTH];

// redondeo simétrico (away-from-zero)
wire signed [OUT_WIDTH-1:0] rstep =
  rb ? (y_norm[ACC_WIDTH-1] ? -{{(OUT_WIDTH-1){1'b0}},1'b1}
                            :  {{(OUT_WIDTH-1){1'b0}},1'b1})
     : {OUT_WIDTH{1'b0}};

assign out_data = y_slice + rstep;
